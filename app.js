...
(function(){
  ...
  if(media){ window.addEventListener('scroll',()=>{ media.style.transform = `translateY(${window.scrollY*0.18}px)`; },{passive:true}); }
  ...
  // Countdown to next first Saturday 8am
  function nextFirstSaturday(){
    ...
    let d=firstSat(y,m); if(d<=now){ m++; if(m>11){m=0;y++;} d=firstSat(y,m); }
    return d;
  }
  function tick(){
  ...
  // Demo checkout modal (placeholder until Stripe is wired)
